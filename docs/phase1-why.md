...
> **⚑ 2026-07-30 修正**：当时把这 ±2 归因为「不可解释的噪声」。Phase 2 的 time-travel 实验
> 找到了候选机制：**模型的加载状态会改变输出**。同一份输入，冷启动 3/3 给一个答案、
> 热缓存 3/3 给另一个答案（交替顺序验证，已排除时间漂移，见 `docs/phase2-why.md`）。
> 本节这批实验没有控制 `keep_alive`、也没有预热，所以噪声里至少有一部分来自加载状态变化。
> **可复现的评测必须固定 `keep_alive` 并预热。**
